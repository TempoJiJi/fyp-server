...
FloorplanFilesystem.prototype.update = function(i, cup, notes, text)
{
	...
	for (var i = 0; i < data.nodeDataArray.length; i++){
		...
		for (var j = 1; j < notes.length; j++) {
			if (notes[j] == cup_id) {
				...
				var text = data.nodeDataArray[i].text.split("-");
				text[0]--;
				text[1]--;
				this.update(i, cup, notes, text);
				return;
			}
		}
	}
...
FloorplanFilesystem.prototype.service = function(cup_id, beacon_id, cup_info)
{
	...
	for (var i = 0; i < data.nodeDataArray.length; i++){
		...
		if (notes[0] == beacon_id) {	// Table found
			...
			if (flag == 0) {
				var cup = data.nodeDataArray[i].cup + 1;
				var new_notes = data.nodeDataArray[i].notes + "\n" + cup_id;
				//TODO: update cup info here/////////////////
				var text = cup + "-0";
				///////////////////////////
				this.remove(cup_id, i);
				this.update(i, cup, new_notes, text);
			}
			if (flag == 1) {
				//TODO: update cup info here
			}
